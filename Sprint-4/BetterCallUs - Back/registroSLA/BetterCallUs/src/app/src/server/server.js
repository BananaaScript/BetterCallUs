...
app.get('/areas', async (req, res) => {
  const connection = await connect();
  const [rows] = await connection.execute('SELECT area1 FROM areas');

  const nomesAreas = rows.map((area) => area.area1);
...
app.post('/chamados', async (req, res) => {
  const { nome_area, titulo, sumario, nome_equipamento} = req.body; 
...
/*   switch (area) {
  ...
  const connection = await connect();
  await connection.execute('INSERT INTO chamado (nome_area, titulo, sumario, tempoderesposta, status, nome_equipamento, prioridade) VALUES (?, ?, ?, ?, ?, ?, ?)', [nome_area || null, titulo, sumario,  tempoderesposta, status, nome_equipamento, prioridade]);
...
app.get('/registroArea', async (req, res) => {
  const connection = await connect();
...
app.post('/registroArea', async(req, res) => {
  const {id, area1, prioridade, descricao, tempoDeResposta} = req.body;

  const connection = await connect();
  await connection.execute('INSERT INTO areas(id, area1, prioridade, descricao, tempoDeResposta) VALUES (?, ?, ?, ?, ?)', [id, area1, prioridade, descricao, tempoDeResposta]);

  res.json({message: 'Área registrada com sucesso'});
})
